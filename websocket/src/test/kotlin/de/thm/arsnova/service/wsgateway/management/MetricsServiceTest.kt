...
class MetricsServiceTest {
  ...
  private fun testHandleRoomUserCountChangedEvent(maxUserCount: Int) {
    ...
    for (i in MetricsService.ACTIVE_ROOM_MIN_USERS..maxUserCount) {
      ...
      applicationEventPublisher.publishEvent(RoomUserCountChangedEvent(
        roomId = roomId,
        count = i
      ))
      val before = LocalDateTime.now().plusSeconds(1)
      if (i < MetricsService.ACTIVE_ROOM_MIN_USERS) {
        assertNull(metricsService.activeRooms[roomId],
          "Room should not have metrics.")
        continue
      }
      assertEquals(i, metricsService.activeRooms[roomId]?.maxUserCount,
        "'maxUserCount' does not have the expected value.")
      assertNotNull(metricsService.activeRooms[roomId]?.sessionStart,
        "'sessionStart' is expected to be not null.")
      assertTrue(metricsService.activeRooms[roomId]?.sessionStart?.isAfter(after)!!,
        "'sessionStart' is not in expected time range.")
      assertTrue(metricsService.activeRooms[roomId]?.sessionStart?.isBefore(before)!!,
        "'sessionStart' is not in expected time range.")
      assertEquals(-1, metricsService.activeRooms[roomId]?.decliningMinUserCount,
    ...
    for (i in maxUserCount downTo max(
      ...
      if (i < maxUserCount * MetricsService.SESSION_END_USER_FACTOR
        ...
        assertEquals(maxUserCount, metricsService.activeRooms[roomId]?.maxUserCount,
          "'maxUserCount' does not have the expected value.")
        assertEquals(-1, metricsService.activeRooms[roomId]?.decliningMinUserCount,
          "'decliningMinUserCount' does not have the expected value.")
      }
