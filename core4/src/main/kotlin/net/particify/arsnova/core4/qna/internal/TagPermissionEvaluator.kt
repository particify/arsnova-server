...
@Component
class TagPermissionEvaluator(private val tagRepository: TagRepository) :
    DomainPermissionEvaluator<Tag> {
  private val logger = LoggerFactory.getLogger(this::class.java)

  override fun hasPermission(
      user: User,
      targetDomainObject: Tag,
      permission: Any
  ...
  override fun findOneByKey(key: Any): Tag {
    check(key is UUID) { "Excepted key to be of type UUID." }
    return tagRepository.findByIdOrNull(key) ?: throw QnaNotFoundException(key)
  }

  override fun supports(clazz: KClass<out Any>) = clazz == Tag::class

  override fun supports(className: String) = className == "Tag"
}
