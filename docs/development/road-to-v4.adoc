...
Tackling these issues is not an easy feat because they are rooted in architectural and technology stack choices.
...
CouchDB has to analyze all document changes for updating view because it does not know which ones are relevant.
...
There are libraries around for CouchDB but, at least on the Java side, multiple have been abandoned (Ektorp, CouchDB4J, java-cloudant).
...
==== NoSQL
CouchDB is a document-based DBMS which does not enforce any schema on its JSON documents.
...
Working with this new architecture over a longer period of time has shown multiple shortcomings.
...
* The port of tooling for the development of microservices is limited.
...
The limitations become noticeable in more complex scenarios.
...
==== Inconsistencies
There are currently some inconsistencies in the structure of endpoints:
...
=== Technical debt
There a few widespread issues in the code base which do not cause problems on its own but can lead to the introduction of bugs.
...
This is a bad practise and requires workarounds to initialize the services.
