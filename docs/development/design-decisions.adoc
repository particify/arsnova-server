...
Furthermore, the language has better defaults and requires less boilerplate code which leads to better readability and reduced maintenance work.
...
Furthermore, Java-based framework do not work well with some Kotlin features.
E.g. inline value classes can cause issues with frameworks which use reflection.

Overall, we came to the conclusion that Kotlin's advantages overweight its disadvantages for our use case.
...
The newer Mango views make it easier to query but lack features of the older JS-based views (e.g. basic aggregation functionality like count and sum).
...
PostgreSQL has been around for a long time (since 1996) and has a reliable release cycle.
It has a large ecosystem with integrations for most programming languages and frameworks.
...
Furthermore, it brings additional safety by avoiding direct SQL queries which cannot be checked at compile time.
...
REST APIs are great due to their simplicity and cacheability.
...
But as a result, the API is no longer self explanatory.
Furthermore, the use of different views single resources makes it hard to define reliable data types on the client side.
...
Furthermore, code generators are available, which makes it easy to keep client types in sync with the backend and allows type safe us of API data.
